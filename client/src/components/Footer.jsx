const Footer = () => {
  return (
    <div className="mt-20 w-auto">
      <div className="font-general-sans flex flex-col justify-center items-center">
        <h2 className="font-bold text-[18px]">Follow Us</h2>
        <div className="flex gap-10 py-4">
          <img
            className="w-[30px] h-[30px]"
            src="https://generation-sessions.s3.amazonaws.com/95c2b3830161ceb2a4718a1dbaf13df6/img/vector.svg"
          />
          <img
            className="w-[30px] h-[30px]"
            src="https://generation-sessions.s3.amazonaws.com/95c2b3830161ceb2a4718a1dbaf13df6/img/vector-1.svg"
          />
        </div>
      </div>
      <div className="mt-5 flex px-10 flex-col text-center mb-4 items-center justify-center w-full gap-y-5">
        <div className="mr-2-md flex flex-wrap gap-2 justify-center items-center">
          <img
            src="//d26lpennugtm8s.cloudfront.net/assets/common/img/logos/payment/new_logos_payment/visa@2x.png"
            className="w-[40px] h-[25px]"
            alt="visa"
          />
          <img
            src="//d26lpennugtm8s.cloudfront.net/assets/common/img/logos/payment/new_logos_payment/mastercard@2x.png"
            className="w-[40px] h-[25px]"
            alt="mastercard"
          />
          <img
            src="//d26lpennugtm8s.cloudfront.net/assets/common/img/logos/payment/new_logos_payment/amex@2x.png"
            className="w-[40px] h-[25px]"
            alt="amex"
          />
          <img
            src="//d26lpennugtm8s.cloudfront.net/assets/common/img/logos/payment/new_logos_payment/diners@2x.png"
            className="w-[40px] h-[25px]"
            alt="diners"
          />
          <img
            src="//d26lpennugtm8s.cloudfront.net/assets/common/img/logos/payment/new_logos_payment/paypal@2x.png"
            className="w-[40px] h-[25px]"
            alt="paypal"
          />
          <img
            src="//d26lpennugtm8s.cloudfront.net/assets/common/img/logos/payment/new_logos_payment/ar/banelco@2x.png"
            className="w-[40px] h-[25px]"
            alt="ar_banelco"
          />
          <img
            src="//d26lpennugtm8s.cloudfront.net/assets/common/img/logos/payment/new_logos_payment/ar/cabal@2x.png"
            className="w-[40px] h-[25px]"
            alt="ar_cabal"
          />
          <img
            src="//d26lpennugtm8s.cloudfront.net/assets/common/img/logos/payment/new_logos_payment/ar/tarjeta-naranja@2x.png"
            className="w-[40px] h-[25px]"
            alt="ar_tarjeta-naranja"
          />
          <img
            src="//d26lpennugtm8s.cloudfront.net/assets/common/img/logos/payment/new_logos_payment/ar/tarjeta-shopping@2x.png"
            className="w-[40px] h-[25px]"
            alt="ar_tarjeta-shopping"
          />
          <img
            src="//d26lpennugtm8s.cloudfront.net/assets/common/img/logos/payment/new_logos_payment/mercadopago@2x.png"
            className="w-[40px] h-[25px]"
            alt="mercadopago"
          />
          <img
            src="//d26lpennugtm8s.cloudfront.net/assets/common/img/logos/payment/new_logos_payment/ar/argencard@2x.png"
            className="w-[40px] h-[25px]"
            alt="ar_argencard"
          />
          <img
            src="//d26lpennugtm8s.cloudfront.net/assets/common/img/logos/payment/new_logos_payment/pagofacil@2x.png"
            className="w-[40px] h-[25px]"
            alt="pagofacil"
          />
          <img
            src="//d26lpennugtm8s.cloudfront.net/assets/common/img/logos/payment/new_logos_payment/rapipago@2x.png"
            className="w-[40px] h-[25px]"
            alt="rapipago"
          />
        </div>
        <div className="mr-2-md flex flex-wrap gap-2 justify-center items-center">
          <img
            src="//d26lpennugtm8s.cloudfront.net/assets/common/img/logos/shipping/ar/oca@2x.png"
            className="w-[40px] h-[25px]"
            alt="ar_oca"
          />
          <img
            src="//d26lpennugtm8s.cloudfront.net/assets/common/img/logos/shipping/api/1484@2x.png"
            className="w-[40px] h-[25px]"
            alt="api_1484"
          />
          <img
            src="//d26lpennugtm8s.cloudfront.net/assets/common/img/logos/shipping/api/2682@2x.png"
            className="w-[40px] h-[25px]"
            alt="api_2682"
          />
          <img
            src="//d26lpennugtm8s.cloudfront.net/assets/common/img/logos/shipping/api/2423@2x.png"
            className="w-[40px] h-[25px]"
            alt="api_2423"
          />
          <img
            src="//d26lpennugtm8s.cloudfront.net/assets/common/img/logos/shipping/branch@2x.png"
            className="w-[40px] h-[25px]"
            alt="branch"
          />
          <img
            src="//d26lpennugtm8s.cloudfront.net/assets/common/img/logos/shipping/api/1211@2x.png"
            className="w-[40px] h-[25px]"
            alt="api_1211"
          />
        </div>
      </div>
      <div className="mt-10 font-general-sans text-[13px] text-[#616161] flex justify-center items-center flex-col">
        <h2>© HYBRD - { new Date().getFullYear() }. All rights reserved.</h2>
        <h2>Designed by Mauricio Villanueva</h2>
      </div>
    </div>
  );
};

export default Footer;
